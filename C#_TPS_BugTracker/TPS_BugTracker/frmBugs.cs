...
namespace TPS_BugTracker
{
    public partial class frmBugs : Form
    {
        ...
        private void btnConsultar_Click(object sender, EventArgs e)
        {
            ...
            llenarGrilla(BDHelper.getBDHelper().ConsultaSqlConParametros(strSql, parametros), dgvBugs);
        }
        ...
        private void llenarGrilla(DataTable tabla, DataGridView grilla)
        {
            ...
            if(tabla.Rows.Count>0)
            {
                ...
                {
                    grilla.Rows.Add(tabla.Rows[i]["id_bug"], tabla.Rows[i]["titulo"], tabla.Rows[i]["id_producto"], tabla.Rows[i]["fecha_alta"], tabla.Rows[i]["id_estado"],
                        tabla.Rows[i]["id_criticidad"], tabla.Rows[i]["id_prioridad"]);
                }
            }
        }
    }
